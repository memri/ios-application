...
class Coordinator<Item, Content: View> : NSObject, UITableViewDelegate, UITableViewDataSource where Item: AnyObject {
    ...
    init(parent: TableView<Item,Content>) {
        ...
        self.tableViewController.tableView.tableHeaderView = UIView(frame: .zero)
        self.tableViewController.tableView.tableFooterView = UIView(frame: .zero)
    ...
    public var renderConfig: CascadingListConfig? {
        self.parent.main.cascadingView.renderConfig as? CascadingListConfig
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt: IndexPath) {
        if Item.self == DataItem.self {
            ...
            if let press = self.renderConfig?.press {
                self.parent.main.executeAction(press, with: dataItem)
            }
        }
    }
    ...
    private func deleteAction(deleteItemAtRow indexPath: IndexPath) -> UIContextualAction {
        ...
        let delete = UIContextualAction(style: .destructive, title:  "Delete", handler: { (ac: UIContextualAction, view: UIView, success: (Bool) -> Void) in
            ...
            if Item.self == DataItem.self {
                self.parent.main.executeAction( Action(self.parent.main, "delete"), with: self.parent.main.items[itemAtRow.row])
