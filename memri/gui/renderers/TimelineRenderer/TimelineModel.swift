...
struct TimelineModel {
    ...
    init(
        dataItems: [Item],
        itemDateTimeResolver: @escaping ((Item) -> Date?),
    ...
    static func group(
        ...
        var sortedGroups = groupedByDay.compactMap { (date, items) -> TimelineGroup? in
            ...
            return TimelineGroup(
                date: date,
                items: group(items, collapseByTypeWhereCountGreaterThan: maxCount)
            )
        ...
        let sortedGroups = groupedByType.flatMap { (type, items) -> [TimelineElement] in
            if items.count > maxCount {
                ...
                return items.indexed()
                    .map { TimelineElement(itemType: type, index: $0.index, items: [$0.element]) }
            }
