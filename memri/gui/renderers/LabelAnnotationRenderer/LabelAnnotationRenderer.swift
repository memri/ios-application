...
class LabelAnnotationRendererController: RendererController, ObservableObject {
    static let rendererType = RendererType(
        ...
        makeController: LabelAnnotationRendererController.init,
        makeConfig: LabelAnnotationRendererController.makeConfig
    )
    ...
    var labelOptions: [LabelOption] {
        config.labelOptions.indexed().map { label in
            LabelOption(
                labelID: label.element,
                text: label.element.titleCase(),
                icon: Image(systemName: config.labelOptionIcons[safe: label.index] ?? "tag")
            )
        }
    }
    ...
    func currentAnnotation() -> LabelAnnotation? {
        DatabaseController.sync { _ in
            let edge = self.currentItem?.reverseEdges("annotatedItem")?
                .first(where: { $0.source(type: LabelAnnotation.self)?.labelType == labelType })
