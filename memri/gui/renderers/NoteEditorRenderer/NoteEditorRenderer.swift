...
class NoteEditorRendererController: RendererController, ObservableObject {
    static let rendererType = RendererType(
        name: "noteEditor",
        icon: "doc.richtext",
        makeController: NoteEditorRendererController.init,
        makeConfig: NoteEditorRendererController.makeConfig
    )
...
struct NoteEditorRendererView: View {
    ...
    var body: some View {
        MemriTextEditor(
            model: { [weak controller] in controller?.getEditorModel() ?? MemriTextEditorModel() },
        ...
        .sheet(isPresented: $controller.showingImagePicker) {
            ImagePickerView(
                sourceType: controller
                    .showingImagePicker_shouldUseCamera ? .camera : .photoLibrary,
