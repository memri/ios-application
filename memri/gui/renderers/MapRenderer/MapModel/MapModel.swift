...
class MapModel {
    ...
    init() {
        updateQueue.throttle(for: .seconds(0.1), scheduler: RunLoop.main, latest: true)
            .sink { [weak self] in
                self?._updateModel()
            }
    ...
    func lookupAddress(_ address: Address) -> CLLocation? {
        let (currentResult, lookupPublisher) = MapHelper.shared
        ...
        if addressLookupCancellables[address] == nil {
            addressLookupCancellables[address] = lookupPublisher?
                .sink { [weak self] _ in self?.updateModel() }
        }
        return nil
    }
