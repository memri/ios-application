...
class FileRendererController: RendererController, ObservableObject {
    static let rendererType = RendererType(
        name: "fileViewer",
        icon: "doc.text",
        makeController: FileRendererController.init,
    ...
    var files: [FileViewerItem] {
        context.items.compactMap { item -> FileViewerItem? in
            guard let file = resolveExpression(config.file, toType: File.self, forItem: item) ??
                (item as? File)
            else { return nil }
            return FileViewerItem(url: file.url,
                                  title: resolveExpression(
                                      config.itemTitle,
                                      toType: String.self,
                                      forItem: item
                                  ))
...
struct FileRendererView: View {
    ...
    var body: some View {
        MemriFileViewController(
            ...
            navBarHiddenBinding: controller.isFullScreen
        )
    }
}
