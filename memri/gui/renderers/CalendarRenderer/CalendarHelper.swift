...
struct CalendarHelper {
    ...
    func getMonths(from startDate: Date, to endDate: Date) -> [Date] {
        ...
        var dates: [Date] = []
        calendar.enumerateDates(startingAfter: firstMonth.addingTimeInterval(-1), matching: DateComponents(day: 1), matchingPolicy: .nextTime) { (date, match, stop) in
            if let date = date {
                dates.append(date)
    ...
    func getDays(forMonth month: Date) -> [Date] {
        ...
        var dates: [Date] = []
        calendar.enumerateDates(startingAfter: month.addingTimeInterval(-1), matching: DateComponents(hour: 0), matchingPolicy: .nextTime) { (date, match, stop) in
            if let date = date {
                dates.append(date)
