...
class MessageRendererController: RendererController, ObservableObject {
    static let rendererType = RendererType(
        name: "messages",
        icon: "message",
        makeController: MessageRendererController.init,
        makeConfig: MessageRendererController.makeConfig
    )
...
struct MessageRendererView: View {
    ...
    var section: ASSection<Int> {
        ASSection<Int>(
            ...
            selectionMode: .selectMultiple(controller.context.selectedIndicesBinding)
        ) { item, cellContext in
            ...
            self.controller.view(for: item) // , ViewArguments(["previousItem": previousItem]))
                .padding(EdgeInsets(top: cellContext.isFirstInSection ? 0 : self.controller.config
                        .spacing.height / 2,
                    leading: self.controller.config.edgeInset.left,
                    bottom: cellContext.isLastInSection ? 0 : self.controller.config
                        .spacing.height / 2,
    ...
    var messageComposer: some View {
        HStack(spacing: 6) {
            MemriFittedTextEditor(
                contentBinding: $controller.composedMessage,
        ...
        .padding(.vertical, 5)
        .padding(.leading, min(
            max(self.controller.config.edgeInset.left, 5),
            15
        )) // Follow user-defined insets where within a reasonable range
        .padding(.trailing, min(
            max(self.controller.config.edgeInset.right, 5),
            15
        )) // Follow user-defined insets where within a reasonable range
