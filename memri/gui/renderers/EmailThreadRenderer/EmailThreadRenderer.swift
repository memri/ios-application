...
class EmailThreadRendererController: RendererController, ObservableObject {
    static let rendererType = RendererType(name: "emailThread", icon: "envelope.fill", makeController: EmailThreadRendererController.init, makeConfig: EmailThreadRendererController.makeConfig)
    ...
    func getEmailItems() -> [EmailThreadItem] {
        context.items.map { item in
            EmailThreadItem(uuid: item.uid.value.map(String.init) ?? UUID().uuidString,
                            contentHTML: resolveExpression(config.content, toType: String.self, forItem: item) ?? "",
