...
struct ConfigPanel: View {
    ...
    var body: some View {
        ...
        .overlay(RoundedRectangle(cornerRadius: 15)
            .strokeBorder(shouldMoveAboveKeyboard ? Color(.systemFill) : .clear))
        .modifier(KeyboardModifier(enabled: shouldMoveAboveKeyboard,
                                   overrideHeightWhenVisible: 120))
    }
        ...
        if configItem.isItemSpecific {
            ...
            case .special(.chartType):
                return NavigationLink(
                    destination: ConfigPanelEnumSelectionView(configItem: configItem,
                                                              type: ChartType.self)
                ) {
                    Text(configItem.displayName)
                }.eraseToAnyView()
            case .special(.timeLevel):
                return NavigationLink(destination: ConfigPanelEnumSelectionView(
                    configItem: configItem,
                    type: TimelineModel.DetailLevel.self
                )) {
                    Text(configItem.displayName)
                }.eraseToAnyView()
            default:
                return NavigationLink(destination: ConfigPanelStringView(
                    configItem: configItem,
                    shouldMoveAboveKeyboard: $shouldMoveAboveKeyboard
                )) {
                    Text(configItem.displayName)
                }.eraseToAnyView()
            }
...
private extension ConfigPanel {
    ...
    func getConfigItems() -> [ConfigPanelModel.ConfigItem] {
        currentRendererConfig?.configItems(context: context) ?? []
    }
}
...
struct ConfigPanelBoolView: View {
    ...
    func bindingForBoolExp(configItem: ConfigPanelModel.ConfigItem) -> Binding<Bool> {
        Binding<Bool>(
            get: { [weak context] in context?.currentView?.renderConfig?.cascadeProperty(
...
struct ConfigPanelSelectionView: View {
    ...
    func onSelect(_ selected: ConfigPanelModel.PossibleExpression) {
        context.currentView?.renderConfig?.setState(
            configItem.propertyName,
            Expression(selected.expressionString, startInStringMode: false,
    ...
    var currentSelection: String {
        (context.currentView?.renderConfig?
            .cascadeProperty(configItem.propertyName, type: Expression.self)?.code ?? "")
    ...
    func getRelevantFields() -> [ConfigPanelModel.PossibleExpression] {
        ...
        let propertyOptions = properties
            .compactMap { prop -> ConfigPanelModel.PossibleExpression? in
                if configItem.type.supportedRealmTypes.contains(prop.type),
                   !ConfigPanelSelectionView.excludedFields.contains(prop.name),
                   !prop.name.hasPrefix("_")
                {
        ...
        let computedPropertyOptions = computedProperties
            .compactMap { prop -> ConfigPanelModel.PossibleExpression? in
                if configItem.type.supportedRealmTypes.contains(prop.type) {
                    return ConfigPanelModel.PossibleExpression(
        ...
        return (propertyOptions + computedPropertyOptions)
            .sorted(by: { $0.propertyName < $1.propertyName })
    }
}
...
struct ConfigPanelSortView: View {
    ...
    func getSortFields() -> [ConfigPanelModel.PossibleExpression] {
        ...
        return properties.compactMap { prop -> ConfigPanelModel.PossibleExpression? in
            if !ConfigPanelSelectionView.excludedFields.contains(prop.name),
               !prop.name.hasPrefix("_")
            {
                return ConfigPanelModel.PossibleExpression(propertyName: prop.name)
            }
    ...
    where EnumType.RawValue == String
{
    ...
    var currentSelection: String? {
        context.currentView?.renderConfig?.cascadeProperty(
            configItem.propertyName,
            type: String.self
        )
    }
