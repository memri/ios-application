...
class Coordinator<Item, Content: View>: NSObject, UITableViewDelegate, UITableViewDataSource where Item: AnyObject {
	...
	init(parent: TableView<Item, Content>) {
		...
		tableViewController.tableView.register(HostingTableViewCell<Content>.self, forCellReuseIdentifier: cellIdentifier)
	...
	func tableView(_: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt _: IndexPath) {
	...
	private func shareAction() -> UIContextualAction {
		let share = UIContextualAction(style: .normal, title: "Share", handler: { (_: UIContextualAction, _: UIView, success: (Bool) -> Void) in
	...
	private func favoriteAction() -> UIContextualAction {
		let favorite = UIContextualAction(style: .normal, title: "Favorite", handler: { (_: UIContextualAction, _: UIView, success: (Bool) -> Void) in
	...
	private func deleteAction(deleteItemAtRow indexPath: IndexPath) -> UIContextualAction {
		...
		let delete = UIContextualAction(style: .destructive, title: "Delete", handler: { (_: UIContextualAction, _: UIView, success: (Bool) -> Void) in
