...
class FileStorageController {
	...
	static func getFileStorageURL() -> URL {
		...
		guard let homeDir = ProcessInfo.processInfo.environment["SIMULATOR_HOST_HOME"] else { fatalError("Couldn't find home directory from simulator environment") }
		let memriFileURL = URL(fileURLWithPath: homeDir, isDirectory: true).appendingPathComponent("memriDevData/fileStore", isDirectory: true)
		...
		let memriFileURL = documentsDirectory.appendingPathComponent("fileStore", isDirectory: true)
		createIfDoesntExist(directoryURL: memriFileURL)
		return memriFileURL
		#endif
	}
